...
def train_cpm(ipmat, pheno):
    ...
    num_features = np.sqrt(ipmat.shape[0])

    #cc = [stats.pearsonr(pheno, im) for im in ipmat]
    cc = [stats.spearmanr(pheno, im) for im in ipmat]

    rmat = np.array([c[0] for c in cc])
    pmat = np.array([c[1] for c in cc])
    rmat = np.reshape(rmat, [num_features, num_features])
    pmat = np.reshape(pmat, [num_features, num_features])
